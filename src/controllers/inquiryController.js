  ...
  setImmediate(async () => {
    try {
      const populated = await Inquiry.findById(inquiry._id)
        .populate({ path: 'property', select: 'title listingId propertyType agent', populate: { path: 'agent', select: 'email' } });
      await syncToBigin(
        inquiry,
        populated?.property?.title,
        populated?.property?.listingId,
        populated?.property?.propertyType,
        populated?.property?.agent?.email,
      );
    } catch (err) {
...
exports.retrySyncInquiry = asyncHandler(async (req, res) => {
  ...
  await syncToBigin(
    ...
    inquiry.property?.listingId,
    inquiry.property?.propertyType,
    inquiry.property?.agent?.email,
  );
